...
func NewDialerGroup(
	...
	group.selectedIndex.Store(int32(p.FixedIndex))
...
func (g *DialerGroup) PolicyString() string {
	...
	case consts.DialerSelectionPolicy_Fixed:
		return "fixed(" + strconv.Itoa(state.policy.FixedIndex) + ")"
	case consts.DialerSelectionPolicy_Select:
		return "select(" + strconv.Itoa(int(g.selectedIndex.Load())) + ")"
	case consts.DialerSelectionPolicy_SelectFallback:
		return "select_fallback(" + strconv.Itoa(int(g.selectedIndex.Load())) + ")"
	case consts.DialerSelectionPolicy_MinLastLatency:
		return "min_last_latency"
	case consts.DialerSelectionPolicy_MinAverage10Latencies:
		return "min_avg10"
	case consts.DialerSelectionPolicy_MinMovingAverageLatencies:
		return "min_moving_avg"
	default:
		return "unknown"
	}
}

func (g *DialerGroup) SetSelectedIndex(index int) {
	if index >= 0 && index < len(g.Dialers) {
		g.selectedIndex.Store(int32(index))
	}
}

func (g *DialerGroup) GetSelectedIndex() int {
...
func (g *DialerGroup) _select(networkType *dialer.NetworkType, state *dialerGroupSelectionState, policy DialerSelectionPolicy, excluded *dialer.Dialer) (d *dialer.Dialer, latency time.Duration, selectedNetworkType *dialer.NetworkType, err error) {
	...
	case consts.DialerSelectionPolicy_Select:
		idx := int(g.selectedIndex.Load())
		if idx >= 0 && idx < len(g.Dialers) {
			return g.Dialers[idx], 0, networkType, nil
		}
		return g.Dialers[0], 0, networkType, nil
	...
	case consts.DialerSelectionPolicy_SelectFallback:
		...
		if idx >= 0 && idx < len(g.Dialers) {
			...
			if d.MustGetAlive(networkType) {
				return d, 0, networkType, nil
			}
		}
		// Fall through to min_moving_avg if preferred is unavailable
		fallthrough
...
func policyNeedsAliveState(policy consts.DialerSelectionPolicy) bool {
	...
	case consts.DialerSelectionPolicy_Random,
		...
		return true
	case consts.DialerSelectionPolicy_Fixed,
		consts.DialerSelectionPolicy_Select:
		return false
	case consts.DialerSelectionPolicy_SelectFallback:
		return true
	default:
