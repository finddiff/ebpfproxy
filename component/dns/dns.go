...
func (d *Dns) Upstreams() []DnsUpstreamEntry {
	if d == nil {
		return nil
	}
	var result []DnsUpstreamEntry
	for i, u := range d.upstream {
		if u == nil || u.Raw == nil {
			continue
		}
		tag := ""
		if i < len(d.upstreamTags) {
			tag = d.upstreamTags[i]
		}
		result = append(result, DnsUpstreamEntry{
			Tag:   tag,
			Link:  u.Raw.String(),
			URL:   u.Raw,
			Index: i,
...
func New(dns *config.Dns, opt *NewOption) (s *Dns, err error) {
	...
	for i, upstreamRaw := range dns.Upstream {
		...
		s.upstreamTags = append(s.upstreamTags, tag)
		var u *url.URL
