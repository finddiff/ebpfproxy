...
func (c *ControlPlane) Route(src, dst netip.AddrPort, domain string, l4proto consts.L4ProtoType, routingResult *bpfRoutingResult) (outboundIndex consts.OutboundIndex, mark uint32, must bool, err error) {
	return c.routeWithLRU(src, dst, l4proto, func() (consts.OutboundIndex, uint32, bool, int, error) {
		...
		bSrc := src.Addr().As16()
		bDst := dst.Addr().As16()
		outboundIndex, mark, must, ruleIndex, err := c.routingMatcher.MatchWithIndex(
			bSrc,
			bDst,
			src.Port(),
			dst.Port(),
		...
		return outboundIndex, mark, must, ruleIndex, err
	})
}
