...
func newControlPlaneWithContextOptions(
	...
	// Save original rules for WebUI display before releasing memory
	var savedOriginalRules []OriginalRuleInfo
	for _, rule := range routingA.Rules {
		if rule != nil {
			outbound := ruleOutboundToName(rule.Outbound)
			ruleText := rule.String(false, true, true)
			// Strip the ->outbound suffix since we store outbound separately
			if idx := strings.LastIndex(ruleText, "->"); idx > 0 {
				ruleText = ruleText[:idx]
			}
			savedOriginalRules = append(savedOriginalRules, OriginalRuleInfo{
				Rule:     ruleText,
				Outbound: outbound,
			})
		}
	}
	if routingA.Fallback != nil {
		outbound := ruleOutboundToName(routingA.Fallback)
		savedOriginalRules = append(savedOriginalRules, OriginalRuleInfo{
			Rule:     "fallback",
			Outbound: outbound,
		})
	...
	dnsControllerOption.Hosts = parseHostsConfig(dnsConfig.Hosts)
	...
	// Start event ringbuf reader for direct connection metadata enrichment
	plane.startEventRingbufReader()
	return plane, nil
}
...
func parseHostsConfig(ks []config.KeyableString) map[string]string {
	if len(ks) == 0 {
		return nil
	}
	...
	for _, k := range ks {
		...
		ip = strings.Trim(ip, "'\"")
		if domain != "" && ip != "" {
			m[strings.TrimSuffix(domain, ".")] = ip
		}
	}
	return m
}
...
func (c *ControlPlane) dnsControllerOption() *DnsControllerOption {
	...
	return &DnsControllerOption{
		...
		CacheAccessCallback: func(cache *DnsCache) (err error) {
			...
			c.storeIPDomainFromCache(cache)
			return nil
		},
...
func (c *ControlPlane) startClosedConnCleanup(ctx context.Context) {
...
// storeIPDomainFromCache populates ipDomainMap (IP->domain) from a DNS cache entry.
// Called incrementally whenever a DNS response is cached, avoiding full cache scans.
func (c *ControlPlane) storeIPDomainFromCache(cache *DnsCache) {
	if c == nil || cache == nil {
		return
	}
	fqdn := cache.GetFqdn()
	...
	fqdn = dnsmessage.CanonicalName(fqdn)
	for _, rr := range cache.Answer {
		hdr := rr.Header()
		var ip string
		switch hdr.Rrtype {
		case dnsmessage.TypeA:
			if a, ok := rr.(*dnsmessage.A); ok && a.A != nil {
				ip = a.A.String()
			}
		case dnsmessage.TypeAAAA:
			if aaaa, ok := rr.(*dnsmessage.AAAA); ok && aaaa.AAAA != nil {
				ip = aaaa.AAAA.String()
			}
		}
		if ip != "" {
			c.ipDomainMap.Store(ip+":0", fqdn)
		}
	}
}
