...
func (c *ControlPlane) storeTcpConnMetadata(lConn net.Conn, src, dst netip.AddrPort, domain string, res *proxyDialResult, dialParam *proxyDialParam) {
	if c.connMetadata == nil {
		return
	}
	ruleIndex := -1
	if c.routingMatcher != nil && domain != "" {
		...
		var mac16 [16]uint8
		copy(mac16[10:], dialParam.Mac[:])
		_, _, _, ruleIndex, _ = c.routingMatcher.MatchWithIndex(
			src.Addr().As16(), dst.Addr().As16(),
			src.Port(), dst.Port(),
			ipVersion, consts.L4ProtoType_TCP, domain,
			dialParam.ProcessName, dialParam.Dscp, mac16,
		)
	}
	meta := ConnMetadata{
		Src:       src.String(),
		...
		RuleIndex: ruleIndex,
		StartTime: time.Now(),
		ID:        lConn.LocalAddr().String() + "->" + lConn.RemoteAddr().String(),
	}
	if res != nil && res.Outbound != nil {
		meta.Outbound = res.Outbound.Name
		meta.Policy = string(res.Outbound.GetSelectionPolicy())
		meta.Network = res.OrigNetworkType
	}
	if res != nil && res.Dialer != nil && res.Dialer.Property() != nil {
		meta.Dialer = res.Dialer.Property().Name
	}
	c.connMetadata.Store(lConn, meta)

	// Populate IP->domain mapping for direct connection enrichment
	if domain != "" {
		key := fmt.Sprintf("%s:%d", dst.Addr().String(), dst.Port())
		c.ipDomainMap.Store(key, domain)
	}
}
...
func (c *ControlPlane) handleConn(ctx context.Context, lConn net.Conn) (err error) {
	...
	// Store connection metadata for WebUI
	c.storeTcpConnMetadata(lConn, src, dst, domain, res, dialParam)
	...
	// Create per-connection transfer tracker
	connTransfer := &connTransfer{}
	c.connTransfers.Store(lConn, connTransfer)
	defer func() { c.connTransfers.Delete(lConn) }()

	downloadRecorder := func(n int64) {
		c.runtimeDownloadRecorder()(n)
		connTransfer.recordDownload(n)
	}
	uploadRecorder := func(n int64) {
		c.runtimeUploadRecorder()(n)
		connTransfer.recordUpload(n)
	}

	if err = RelayTCPContextWithRecords(ctx, lRelayConn, rConn, downloadRecorder, uploadRecorder); err != nil {
