...
type UdpEndpointSnapshot struct {
	Key      UdpEndpointKey
	Endpoint *UdpEndpoint
}

func (p *UdpEndpointPool) Snapshot() []UdpEndpointSnapshot {
	if p == nil {
		return nil
	}
	var snapshots []UdpEndpointSnapshot
	for i := range p.shards {
		shard := &p.shards[i]
		shard.mu.RLock()
		for k, v := range shard.pool {
			snapshots = append(snapshots, UdpEndpointSnapshot{Key: k, Endpoint: v})
		}
		shard.mu.RUnlock()
	}
	return snapshots
