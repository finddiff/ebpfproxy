...
func (m *RoutingMatcher) Match(
	...
	outboundIndex, mark, must, _, err = m.MatchWithIndex(sourceAddr, destAddr, sourcePort, destPort, ipVersion, l4proto, domain, processName, dscp, mac)
	return
}

// MatchWithIndex returns the matched rule index in addition to standard results.
func (m *RoutingMatcher) MatchWithIndex(
	sourceAddr [16]uint8,
	destAddr [16]uint8,
	sourcePort uint16,
	destPort uint16,
	...
	dscp uint8,
	mac [16]uint8,
) (outboundIndex consts.OutboundIndex, mark uint32, must bool, ruleIndex int, err error) {
	if len(sourceAddr) != net.IPv6len || len(destAddr) != net.IPv6len || len(mac) != net.IPv6len {
		return 0, 0, false, 0, fmt.Errorf("bad address length")
	}
	...
	if len(matches) == 0 {
		return 0, 0, false, 0, fmt.Errorf("no compiled routing match set")
	}
	...
	for i, match := range matches {
		...
		case consts.MatchType_IpSet, consts.MatchType_SourceIpSet, consts.MatchType_Mac:
			...
			if lpmIndex < 0 || lpmIndex >= len(m.lpmMatcher) {
				return 0, 0, false, 0, fmt.Errorf("bad lpm index: %d", lpmIndex)
			}
		...
		case consts.MatchType_Fallback:
			goodSubrule = true
		default:
			return 0, 0, false, 0, fmt.Errorf("unknown match type: %v", match.matchType)
		}
	beforeNextLoop:
		...
		if outbound&consts.OutboundLogicalMask !=
			...
			if !badRule {
				...
				return outbound, match.mark, match.must || must, i, nil
			}
			badRule = false
		}
	}
	return 0, 0, false, 0, fmt.Errorf("no match set hit")
}
