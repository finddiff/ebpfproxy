...
func TestConnStateStructSizes(t *testing.T) {
	wantSz := uintptr(72) // 56 + 8(tx_bytes) + 8(rx_bytes)
	if got := unsafe.Sizeof(bpfTcpConnState{}); got != wantSz {
		t.Fatalf("sizeof(bpfTcpConnState) = %d, want %d", got, wantSz)
	}
	if got := unsafe.Sizeof(bpfUdpConnState{}); got != wantSz {
		t.Fatalf("sizeof(bpfUdpConnState) = %d, want %d", got, wantSz)
	}
