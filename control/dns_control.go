...
func (c *DnsController) RuntimeState() *dnsControllerRuntimeState {
	if c == nil {
		return nil
	}
	return c.runtimeState.Load()
...
func (s *dnsControllerRuntimeState) Dns() *dns.Dns {
	if s == nil {
		return nil
	}
	return s.routing
...
func (c *DnsController) HandleWithResponseWriter_(ctx context.Context, dnsMessage *dnsmessage.Msg, req *udpRequest, responseWriter dnsmessage.ResponseWriter) (err error) {
	...
	// Check hosts map first
	if len(dnsMessage.Question) > 0 && !dnsMessage.Response && len(c.hosts) > 0 {
		q := dnsMessage.Question[0]
		qname := q.Name
		hkey := strings.TrimSuffix(strings.ToLower(qname), ".")
		if ip, ok := c.hosts[hkey]; ok {
			resp := new(dnsmessage.Msg)
			resp.SetReply(dnsMessage)
			resp.Authoritative = true
			if ip4 := netip.MustParseAddr(ip); ip4.Is4() && q.Qtype == dnsmessage.TypeA {
				resp.Answer = []dnsmessage.RR{&dnsmessage.A{Hdr: dnsmessage.RR_Header{Name: qname, Rrtype: dnsmessage.TypeA, Class: dnsmessage.ClassINET, Ttl: 3600}, A: ip4.AsSlice()}}
			} else if ip6 := netip.MustParseAddr(ip); ip6.Is6() && q.Qtype == dnsmessage.TypeAAAA {
				resp.Answer = []dnsmessage.RR{&dnsmessage.AAAA{Hdr: dnsmessage.RR_Header{Name: qname, Rrtype: dnsmessage.TypeAAAA, Class: dnsmessage.ClassINET, Ttl: 3600}, AAAA: ip6.AsSlice()}}
			}
			if len(resp.Answer) > 0 {
				if responseWriter != nil {
					return responseWriter.WriteMsg(resp)
				} else if req != nil && req.lConn != nil {
					data, _ := resp.Pack()
					_, _ = req.lConn.Write(data)
				}
				return nil
			}
		}
