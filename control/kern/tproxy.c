...
struct udp_conn_state {
	...
	__u64 tx_bytes;            // Bytes transmitted (client → internet)
	__u64 rx_bytes;            // Bytes received (internet → client)
};
...
enum dae_event_type {
	...
	DAE_EVENT_NEW_DIRECT_CONN = 3,   // New direct connection created
};
		...
		const struct conntrack_args *args)
{
	...
	if (args->outbound == OUTBOUND_DIRECT) {
		send_dae_event(DAE_EVENT_NEW_DIRECT_CONN, args->pid,
			       conntrack_args_pname_or_null(args), args->outbound,
			       key->l4proto, key->sip.u6_addr32,
			       key->dip.u6_addr32, key->sport, key->dport);
	}
   ...
   */
	if (pkt->l4proto == IPPROTO_TCP &&
		...
		if (outbound == OUTBOUND_DIRECT) {
			skb->mark = mark;
			// Fresh lookup required: using the same tcp_state pointer from mark_tcp_seen
