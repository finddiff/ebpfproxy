...
func init() {
	...
	runCmd.PersistentFlags().IntVar(&webuiPort, "webui-port", 0, "Port for the web UI server. Overrides config webui_port. Set 0 to disable.")
...
func (r *Runner) Run() (err error) {
	...
	// Start webui server early to capture all logs.
	var webuiServer *webapi.Server
	effectiveWebuiPort := webuiPort
	if effectiveWebuiPort == 0 {
		effectiveWebuiPort = int(conf.Global.WebuiPort)
	}
	if effectiveWebuiPort != 0 {
		webuiFS := getWebuiFS()
		webuiServer = webapi.NewServer(log, nil, cfgFile, webuiFS)
		if data, err := os.ReadFile(cfgFile); err == nil {
			webuiServer.SetConfigData(data)
		}
		if conf.Global.WebuiToken != "" {
			webuiServer.SetToken(conf.Global.WebuiToken)
		}
		webuiServer.SetOnConfigSave(func() error {
			return unix.Kill(os.Getpid(), unix.SIGUSR1)
		})
		log.AddHook(webapi.NewLogrusHook(webuiServer, logrus.AllLevels))
		webuiAddr := fmt.Sprintf(":%d", effectiveWebuiPort)
		go func() {
			log.Infof("WebUI server starting on http://0.0.0.0%s", webuiAddr)
			if err := webuiServer.ListenAndServe(webuiAddr); err != nil && err != http.ErrServerClosed {
				log.WithError(err).Warn("WebUI server stopped unexpectedly")
			}
		}()
		webuiServer.StartBackgroundUpdates(context.Background())
	}
	...
	// Wire control plane into webui server for live data.
	if webuiServer != nil {
		webuiServer.SetControlPlane(c)
	}
	...
	go func() {
		for req := range reloadManager.reloadReqs {
			...
			// Save user-selected proxy indices for restore after reload
			savedSelectedIndices := c.GetSelectedIndices()
			...
			if stagedHotHandoff {
				...
				c = newC
				c.RestoreSelectedIndices(savedSelectedIndices)
				if webuiServer != nil {
					webuiServer.SetControlPlane(c)
				}
				currCancel = cancel
